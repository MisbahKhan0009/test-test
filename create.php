...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  // Validate mood against allowed set
  $allowed = array_keys(allowed_moods());
  if ($mood !== '' && !in_array($mood, $allowed, true)) {
    $mood = '';
  }

  // Validate date (optional), default to today
  $timestamp = null;
  if ($entryDate !== '') {
    $dt = DateTime::createFromFormat('Y-m-d', $entryDate);
    if ($dt !== false) {
  ...
  if ($title === '' || $content === '') {
    ...
    try {
      $entryId = db_tx(function(PDO $pdo) use ($config, $title, $content, $mood, $timestamp) {
        $stmt = $pdo->prepare('INSERT INTO entries (user_id, title, content, mood, timestamp) VALUES (?, ?, ?, ?, ?)');
        $stmt->execute([current_user_id(), $title, $content, $mood !== '' ? $mood : null, $timestamp]);
        $entryId = (int)$pdo->lastInsertId();

        // Handle uploads inside the same transaction
        if (!empty($_FILES['media']['name'][0])) {
          $dir = ensure_upload_dir(current_user_id());
          $allowed = $config['uploads']['allowed_mime'];
          $max = (int)$config['uploads']['max_size_bytes'];
  ...
  <div class="glass rounded-3xl shadow-2xl p-8 mt-2">
    ...
    <form method="post" enctype="multipart/form-data" class="space-y-5">
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          <label class="block text-sm text-gray-700 mb-1">Date</label>
          <input type="date" name="entry_date" value="<?php echo e($today); ?>" class="w-full rounded-2xl px-4 py-3 bg-white/70 focus:bg-white outline-none border border-primary-100 focus:border-primary-400 shadow-sm transition" />
        </div>
        <div>
          <label class="block text-sm text-gray-700 mb-1">Mood (optional)</label>
          <select name="mood" class="w-full rounded-2xl px-4 py-3 bg-white/70 focus:bg-white outline-none border border-primary-100 focus:border-primary-400 shadow-sm transition">
            <option value="">No mood</option>
            <?php foreach (allowed_moods() as $label => $emoji): ?>
              <option value="<?php echo e($label); ?>"><?php echo e($emoji . ' ' . $label); ?></option>
            <?php endforeach; ?>
      ...
      <div>
        <label class="block text-sm text-gray-700 mb-1">Title</label>
        <input name="title" required class="w-full rounded-2xl px-4 py-3 bg-white/70 focus:bg-white outline-none border border-primary-100 focus:border-primary-400 shadow-sm transition" />
      </div>
      <div>
        <label class="block text-sm text-gray-700 mb-1">Content</label>
        <textarea name="content" required rows="8" class="w-full rounded-2xl px-4 py-3 bg-white/70 focus:bg-white outline-none border border-primary-100 focus:border-primary-400 shadow-sm transition"></textarea>
      </div>
      <div>
        <label class="block text-sm text-gray-700 mb-2">Media (images/audio, optional)</label>
